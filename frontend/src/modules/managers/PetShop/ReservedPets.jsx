...
const ReservedPets = () => {
  ...
  const fetchItems = async (page = 1) => {
    try {
      ...
      const resp = await apiClient.get(`/petshop/manager/inventory/reserved?${qs.toString()}`);
      ...
      console.error('Load reserved pets error:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
        ...
        <Typography variant="h4">Reserved Pets</Typography>
      </Box>
      ...
      <Card>
        <CardContent>
          <Typography variant="h6" gutterBottom>Reserved Pets ({items.length} items)</Typography>
              ...
              <TableBody>
                {items.map(item => (
                  <TableRow key={item._id}>
                    <TableCell>
                      <Chip label={item.petCode || `PET-${item._id.slice(-6)}`} size="small" color="warning" />
                    </TableCell>
                    <TableCell>
          ...
          {items.length === 0 && !loading && (
            <Alert severity="info" sx={{ mt: 2 }}>
              No reserved pets found. Adjust filters or check back later.
            </Alert>
          )}
