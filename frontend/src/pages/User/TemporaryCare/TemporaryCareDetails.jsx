...
const TemporaryCareDetails = () => {
  ...
  useEffect(() => {
    const loadPet = async () => {
      try {
        setLoading(true)
        const response = await temporaryCareAPI.getPetDetails(id)
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{ mb: 3 }}>
            ...
            <CardContent>
              ...
              <Chip 
                label={`In Care Since ${pet.careStartDate ? new Date(pet.careStartDate).toLocaleDateString() : 'Unknown date'}`} 
                color="primary" 
                variant="outlined" 
                sx={{ mt: 1 }}
              />
            ...
            <CardContent>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                ...
                <Typography variant="body2">
                  {pet.careCenter?.address || 'Address not available'}
                </Typography>
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                <PhoneIcon sx={{ mr: 1, color: 'text.secondary' }} />
                <Typography variant="body2">
                  {pet.careCenter?.phone || 'Phone not available'}
                </Typography>
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                <EmailIcon sx={{ mr: 1, color: 'text.secondary' }} />
                <Typography variant="body2">
                  {pet.careCenter?.email || 'Email not available'}
                </Typography>
              </Box>
            </CardContent>
          </Card>
            ...
            <CardContent>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                ...
                <Typography variant="body1">
                  <strong>Start Date:</strong> {pet.careStartDate ? new Date(pet.careStartDate).toLocaleDateString() : 'Not specified'}
                </Typography>
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                <CalendarIcon sx={{ mr: 1, color: 'text.secondary' }} />
                <Typography variant="body1">
                  <strong>End Date:</strong> {pet.careEndDate ? new Date(pet.careEndDate).toLocaleDateString() : 'Not specified'}
                </Typography>
              </Box>
            </CardContent>
          </Card>
...
export default TemporaryCareDetails