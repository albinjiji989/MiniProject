...
const BatchCard = ({ batch, onSelect, onReserve, isFavorite, onFavoriteToggle }) => {
  ...
  if (isBatch) {
    ...
    } else if (batch.samplePets?.length > 0) {
      // Prefer a sample pet matching the displayed gender (if this card represents a gender slice)
      let samplePet;
      if (batch.gender) {
        samplePet = batch.samplePets.find(sp => sp.gender && sp.gender.toLowerCase() === batch.gender.toLowerCase());
      }
      samplePet = samplePet || batch.samplePets[0];
      if (samplePet && samplePet.imageIds?.length > 0) {
        const imgUrl = getImageUrl(samplePet.imageIds[0]);
        if (imgUrl) primaryImage = resolveMediaUrl(imgUrl);
      } else if (samplePet && samplePet.images?.length > 0) {
        const imgUrl = getImageUrl(samplePet.images[0]);
        if (imgUrl) primaryImage = resolveMediaUrl(imgUrl);
      }
    }
  } else {
    // Individual inventory item format
    // First try the populated images array (virtual field)
    if (batch.images?.length > 0) {
      const imgUrl = getImageUrl(batch.images[0]);
      if (imgUrl) primaryImage = resolveMediaUrl(imgUrl);
    } 
    // Then try imageIds (populated Image objects)
