...
const STEPS = [
  { key: 'applicant', label: 'Applicant', path: '/User/adoption/apply/applicant' },
...
export default function ApplyLayout() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <p className="text-sm text-gray-600 mb-4">Fill in your details to apply for adoption.</p>
      <div className="grid grid-cols-5 gap-2 mb-6">
        {STEPS.map((s, i) => (
          <button key={s.key} onClick={()=>navigate(s.path)} className={`px-3 py-2 rounded border text-sm ${i===idx ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-gray-700 border-gray-300'}`}>
            {i+1}. {s.label}
          </button>
        ))}
      </div>
