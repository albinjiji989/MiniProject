...
\begin{abstract}
Pet adoption systems across the world facing major challenges due to their reliance on traditional adoption processes, which results in adoption matching failure with the pet owner. These limitations highlight the need for a more intelligent and data-driven approach to improve adoption outcomes. This paper proposes an AI-driven smart pet adoption system that provide hybrid recommendation models with lightweight blockchain using sha256 hashing method technology. The recommendation system contains a four-model hybrid engine, consisting of Content-Based Filtering using TF-IDF vectorization, SVD-based Collaborative Filtering for analyzing user interactions, XGBoost for predicting adoption success using multiple compatibility features, and K-Means clustering for grouping pets based on behavioral characteristics. In addition, a lightweight blockchain mechanism using SHA-256 hashing is used to ensure secure, transparent, and tamper-proof record management of adoption transactions. The overall system architecture is built using a React frontend, Node.js backend, Python FastAPI microservice, and MongoDB database. The experimental results exceeded our expectations. We managed to reduce adoption failure rates from the typical 60-70\% down to 30-40\%—a significant improvement that means more animals finding permanent homes. Our compatibility prediction model achieved 87\% accuracy with an F1-score of 0.88, indicating strong performance in identifying successful matches. The system's error rate of 13\% represents a substantial improvement over traditional manual assessment methods. With more original previous adoption data we can achieve more accuracy to predict and recommend. Additionally, the lightweight blockchain component demonstrated robust security capabilities, detecting various attack types while maintaining efficient computational performance.
\end{abstract}
...
\hline
\textbf{Hybrid System} & \textbf{0.87} & \textbf{0.86} & \textbf{0.88} & \textbf{0.88} \\
\hline
\end{tabular}
\end{center}
\end{table}
